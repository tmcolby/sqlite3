...
def main():
    ...
    dbFile = 'logs.db'
    dbTable = 'data_log'
    dbColumns = (('Name', 'TEXT'), ('Value', 'REAL'), ('Time', 'REAL'), ('Quality', 'INTEGER'), ('TZ', 'TEXT')) 
    db = Database(dbFile)
    if not db.table:
    ...
    if clearTable == 'clear': db.cursor.execute('DELETE FROM {}'.format(dbTable))
    
    #instantiate plc client and connect
    plcClient = PlcClient('tags.cfg', 'plc.cfg')
    plcClient.connect()
